...
class ExperimentalPhaseLuo1997(
        namedtuple('ExperimentalPhaseLuo1997',
                   ('name', 'JQCH_v', 'xyY_c', 'S_Y_c', 'Y_b', 'Y_r', 'XYZ_o',
                    'metadata'))):
    """
    Defines a *Luo and Rhodes (1997)* *LUTCHI Colour Appearance Data*
    experimental phase.
...
class DatasetLoader_Luo1997(AbstractDatasetLoader):
    ...
    def __init__(self):
        super(DatasetLoader_Luo1997,
              self).__init__(datasets()[DatasetLoader_Luo1997.ID])
...
http://colour.derby.ac.uk/colour/info/lutchi/data/cold65wnl is empty. Mark
        ...
        super(DatasetLoader_Luo1997, self).sync()
        ...
        for group, phases in experimental_groups.items():
            ...
            for (phase, visual_filename, colorimetric_filename, samples_count,
                ...
                if visual_data.shape != colorimetric_data.shape:
                    usage_warning(
                        '"{0}" visual and "{1}" colorimetric file '
                        'have different shape, using first data rows!'.format(
                            visual_filename, colorimetric_filename))
